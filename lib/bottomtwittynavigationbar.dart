library bottom_twitty_navigation_bar;
...
class _BottomNavigationDotBarState extends State<BottomNavigationDotBar>{
  ...
  _afterLayout(_) {
    ...
    setState(() { _positionLeftIndicatorDot = sizeBottomBar.width / (widget.items.length - 1); });
    _indexPageSelected = widget.items.length-2;
  }
  ...
  void _changeOptionBottomBar(int indexPageSelected){
    if(indexPageSelected != _indexPageSelected){
      setState(() { _positionLeftIndicatorDot = (_numPositionBase * ((widget.items.length - (indexPageSelected))))-_numDifferenceBase; });
